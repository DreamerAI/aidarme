...
import { motion, animate, useScroll, useTransform, useSpring, useMotionValue } from 'motion/react';
...
const TECH_STACK = [
  ...
  { label: 'State Manage', items: ['Redux', 'RTK Query','MobX', 'Zustand', 'Effector'] },
  { label: 'Styling', items: ['Tailwind CSS', 'Chakra UI', 'CSS Modules'] },
  { label: 'Backend', items: ['Node.js', 'Express', 'REST API/GraphQL'] },
  { label: 'Tools', items: ['Git', 'Docker', 'Vite', 'Figma', 'ESLint', 'Prettier'] },
...
const InfoSection = ({
  ...
  >
    <motion.div variants={itemFadeUp} className="flex items-baseline gap-3 mb-3">
...
const StatBlock = ({
  ...
  >
    ...
    <span className="font-inter text-[0.65rem] font-semibold tracking-[0.1em] uppercase text-wash-200 leading-snug whitespace-pre-line">
      {label}
    </span>
  </motion.div>
);
...
export const AboutMe = () => {
  ...
  const [bodyRef, bodyInView] = useInView({ triggerOnce: true, threshold: 0.08 });
  const [statsRef, statsInView] = useInView({ triggerOnce: true, threshold: 0.15 });
  const [footerRef, footerInView] = useInView({ triggerOnce: true, threshold: 0.3 });
  ...
  return (
    <section className="relative w-full min-h-screen bg-main-white text-ink-900 px-6 py-10 md:px-10 lg:px-14 xl:px-20 overflow-hidden font-inter">
      {/* ── Header ── */}
      ...
      >
            ...
            {"Aidar Abdykaiymov".split(' ').map((word, i) => (
              <span key={i} className="flex overflow-hidden">
                {word.split('').map((char, j) => (
                  <motion.span key={j} variants={itemFadeUp}>{char}</motion.span>
                ))}
              </span>
            ))}
          </h2>
        ...
        <motion.div variants={itemFadeUp} className="hidden md:flex flex-col items-end gap-1">
      ...
      {/* ── Body ── */}
      <div ref={bodyRef} className="flex flex-col lg:flex-row gap-8 lg:gap-10 xl:gap-12">
        {/* Photo */}
        <motion.div
          variants={fadeRight}
          initial="hidden"
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* 01 — Profile */}
          <InfoSection number="01" title="Profile" delay={0.05} inView={bodyInView}>
            <motion.p variants={itemFadeUp} className="font-inter text-[0.85rem] leading-relaxed text-wash-300 max-w-[440px]">
              Frontend Engineer with 4+ years of commercial experience building scalable products,
              design systems and high-performance web applications. I focus on writing clean code,
              creating great user experiences and solving complex problems. My journey into frontend actually started because I wanted to build a browser-based Pokémon game! Fun fact: my absolute favorite is <TepigTooltip>Tepig</TepigTooltip>.
            </motion.p>
          </InfoSection>

          {/* 02 — Tech Stack */}
          <InfoSection number="02" title="Tech Stack" delay={0.15} inView={bodyInView}>
            <div className="flex flex-col gap-0.5">
              {TECH_STACK.map((row) => (
                <motion.div variants={itemFadeUp} key={row.label} className="flex items-baseline gap-4 py-[3px]">
                  <span className="font-inter text-[0.7rem] sm:text-[0.7rem] font-bold tracking-[0.1em] uppercase text-ink-900 min-w-[100px] sm:min-w-[130px] shrink-0">
                    {row.label}
                  </span>
                  <span className="font-inter text-[0.8rem] text-wash-300 flex flex-wrap gap-x-1">
                    {row.items.map((item, i) => (
                      <React.Fragment key={item}>
                        ...
                        <span className="hover:text-accent-orange transition-colors cursor-default">{item}</span>
                      </React.Fragment>
                    ))}
                  </span>
                </motion.div>
          ...
          {/* 03 — Languages */}
          <InfoSection number="03" title="Languages" delay={0.25} inView={bodyInView}>
              ...
              {LANGUAGES.map((lang, i) => (
                <motion.div
                  ...
                  className={`flex items-baseline gap-2 font-inter ${
                    i < LANGUAGES.length - 1 ? 'pr-6 border-r border-paper-300' : ''
                  }`}
                >
                  <span className="text-[0.85rem] font-bold uppercase text-ink-900">
                    {lang.code}
                  </span>
                  <span className="text-[0.8rem] text-wash-200">{lang.level}</span>
                </motion.div>
              ))}
            </div>
          </InfoSection>
