...
export const PokeballButton = () => {
	...
	const handleMouseMove = (e: React.MouseEvent<HTMLAnchorElement>) => {
		...
		// Subtract position.x and position.y to get the coordinates relative to the original, untranslated button center.
		// This prevents feedback loops and lag when the button moves under the mouse.
		const x = (clientX - (left - position.x + width / 2)) * 0.35;
		const y = (clientY - (top - position.y + height / 2)) * 0.35;
		setPosition({ x, y });
	};
	...
	return (
		<motion.a
			...
			className="relative w-32 h-32 md:w-40 md:h-40 rounded-full flex items-center justify-center overflow-hidden group z-10 shadow-xl"
		>
			<svg
				viewBox="0 0 100 100"
